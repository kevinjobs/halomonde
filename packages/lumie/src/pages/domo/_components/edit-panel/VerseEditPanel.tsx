...
export function VerseEditPanel({post, onSubmit, onCancel, mode}: VerseEditPanelProps) {
  ...
  return (
    <div className={style.verseEdit}>
      ...
      <div className={style.submitVerse}>
        <Button onClick={handleSubmit}>{mode === 'create' ? '新增' : '更新'}</Button>
        <Button onClick={handleCancel} type='error'>取消</Button>
      </div>
    </div>
  )
}